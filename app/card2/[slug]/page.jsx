...
const getLanguageByCode = (code) => {
  ...
  const faqs = [
    {
      question: `How is the distance calculated between ${sourcePlace?.display_name?.split(',')[0] || 'Location'} and ${destinationPlace?.display_name?.split(',')[0] || 'Location'}?`,
      answer: `We calculate the straight-line (as-the-crow-flies) distance using the coordinates of both locations. This gives you the most direct distance in kilometers, miles, and nautical miles.`
    },
    {
      question: "Can I use this tool for any locations worldwide?",
      answer: `Absolutely! Our distance calculator works worldwide. It accurately shows the distance between any two locations on Earth.`
    },
    {
      question: "How accurate is the distance calculation?",
      answer: "The calculation is based on the Haversine formula which provides great-circle distances between two points on a sphere. It's accurate for most purposes but doesn't account for terrain elevation."
    }
  ];
  ...
  return (
    <>
      ...
      <main className="distance-result__container">
        ...
        <section className="distance-result__faq">
          <h2 className="distance-result__section-title">Frequently Asked Questions</h2>
          <div className="distance-result__faq-container">
            {faqs.map((faq, index) => (
              <FAQItem 
                key={index}
